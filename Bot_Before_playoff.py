...
def do_turn(game):
    ...
    volcano_life = game.get_volcano().current_health
...
def handle_portalsIce(game,for_check,sper_mana):
    ...
    if not (game.get_my_living_elves())  and volcano_life > game.get_volcano().current_health and game.get_volcano().damage_by_me < game.get_volcano().damage_by_enemy + game.get_volcano().current_health and game.get_volcano().current_health < game.get_volcano().max_health/2:
...
def try_attack(game,elf,for_check):
    ...
    if optinal_enemy_elves:
        ...
        # if only this elf can attack 
            ...
            return True
    # attack volcano
    if game.get_volcano().is_active() and elf.in_attack_range(game.get_volcano()) and elf in mapElvesPerTurns.keys() and mapElvesPerTurns[elf] == game.get_volcano() and (spell_casted(game,elf) != "speed up" and spell_casted(game,elf) != "both of them" and  spell_casted(game,elf) != "invisibility"):
        if for_check:
            return game.get_volcano()
        elf.attack(game.get_volcano())
