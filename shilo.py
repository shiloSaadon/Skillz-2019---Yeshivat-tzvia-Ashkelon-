...
def handle_portalsIce(game,for_check,sper_mana):
    ...
    for volcano in game.get_active_volcanoes():
        if not (game.get_my_living_elves())  and volcano in volcano_life.keys() and volcano_life[volcano] > volcano.current_health and volcano.damage_by_me < volcano.damage_by_enemy + volcano.current_health and volcano.current_health < volcano.max_health/2:
                ...
                if portal_to_my_castle:
                    ...
                    if portal_to_my_castle[0].currently_summoning == "IceTroll":
                        return True
    # save castle
    all_volcano_attacked = True
    for volcano in game.get_all_volcanoes():
        enemy_elf_to_volcano = sorted(game.get_enemy_living_elves(), key = lambda v: v.distance(volcano))
        if not (volcano in volcano_life.keys() and volcano.current_health < volcano_life[volcano] and volcano not in attack.values() and enemy_elf_to_volcano and enemy_elf_to_volcano[0].in_attack_range(volcano)):
            all_volcano_attacked = False
    if all_volcano_attacked:
        portal_to_my_castle = sorted(game.get_my_portals(), key = lambda e: e.distance(game.get_my_castle()))
        if portal_to_my_castle:
            if for_check:
                return 1
            if not portal_to_my_castle[0].is_summoning and portal_to_my_castle[0].can_summon_ice_troll():
...
def SortElves(game,priority_fauntain,priority_portal):
    ...
    for elf in game.get_my_living_elves():
        ...
        if not elf_in_use and when_I_go_to_volcano(game,elf,True)==1:
            sortdebug(game,"go to attack volcano")
            mapElves[elf] = when_I_go_to_volcano(game,elf,False)
            mapElvesPerTurns[elf] = mapElves[elf]
            elf_in_use = True
        ...
        if not elf_in_use and when_I_go_to_volcano(game,elf,True) :
            sortdebug(game,"go to attack volcano")
            mapElves[elf] = when_I_go_to_volcano(game,elf,False)
            mapElvesPerTurns[elf] = mapElves[elf]
            elf_in_use = True
...
def when_I_go_to_volcano(game,elf,for_check):
    b = False
    volcano_list = []
    for volcano in game.get_all_volcanoes():
        if volcano.is_active() :
    ...
    if for_check:
        return b 
    volcanos = sorted(volcano_list, key = lambda v: v.distance(elf))
    if volcanos:
        return volcanos[0]
...
def try_attack(game,elf,for_check):
    ...
    if optinal_enemy_elves:
        ...
        # if only this elf can attack 
            ...
            return True
    # attack volcano
    optional_volcanos = [volcano for volcano in game.get_active_volcanoes() if elf .in_attack_range(volcano)]
    optional_volcanos.sort(key = lambda v: v.distance(elf))
    if optional_volcanos: 
        if optional_volcanos[0] and elf.in_attack_range(optional_volcanos[0]) and elf in mapElvesPerTurns.keys() and mapElvesPerTurns[elf] in game.get_all_volcanoes() and (spell_casted(game,elf) != "speed up" and spell_casted(game,elf) != "both of them" and  spell_casted(game,elf) != "invisibility"):
            if for_check:
                return optional_volcanos[0]
            elf.attack(optional_volcanos[0])
