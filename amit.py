...
loc_before_disapear = {}  # location before enemy elf used invisibility
...
if_enemy_destroy_my_mana_fountain = False # if enemy destroy my fountain
ok_to_build_fountain = False # after we stop build we want to back build  
fountains_loc = []  # my fountain previous turn
...
enemy_fountain_loc = []  # enemy fountain location - goof places for build portal
...
def do_turn(game):
    ...
    #good places to build mana fountain
    list_fountain=good_locations_for_building_fountain(game)
    
    #the best volcano to elf 
    for elf in game.get_my_living_elves():
        volcanoes=sorted(game.get_all_volcanoes() ,key=lambda v:v.distance(elf))
        my_volcano[elf]=None
        if volcanoes:
            if volcanoes[0].is_active() or volcanoes[0].turns_to_active<3:
                my_volcano[elf]=volcanoes[0]   
            else:
                if len(volcanoes)>1:
                    if volcanoes[0].distance(volcanoes[1])<volcanoes[0].size*2+game.elf_max_speed*7 and volcanoes[1].is_active():
                        my_volcano[elf]=volcanoes[1]
    ...
    # save the location of the enemy mana fountain - good places for build portal
    ...
    # variables for "what to create"
    priority_portal = need_portal(game, True, mapElves) # num of priority for portal
    priority_fountain = need_fountain(game, True, mapElves) # num of priority for mana fountain
    mapElves = SortElves(game, priority_fountain, priority_portal) # get the elves and his targets
    ...
    if game.get_my_portals():
        ...
        priority_tornado = handel_portalstornado(game, True, 0, priority_portal) # num of priority for tornado
    else:
        priority_Ice = 100
        priority_lava = 100
    ...
    if priority_speed_up != 100:
        spare_mana += game.speed_up_cost
    if priority_invisibility != 100:
        spare_mana += game.invisibility_cost
    if priority_tornado == 1:
        spare_mana += game.tornado_cost
    if priority_fountain == 0:
        spare_mana += game.mana_fountain_cost
    # regular debug
    debug(game, " ")
    debug(game, "                         *****MANA*****")
    debug(game, " ")
    ...
    # Using the mana
    WhatToCreate(game, mapElves, priority_Ice, priority_portal, priority_fountain, priority_lava, priority_speed_up,priority_invisibility, priority_tornado, [], spare_mana)
    ...
    for volcano in game.get_all_volcanoes():
        volcano_life[volcano] = volcano.current_health
...
def WhatToCreate(game,mapElves,priority_Ice,priority_portal,priority_fountain,priority_lava,priority_speed_up,priority_invisibility,priority_tornado,without,spare_mana):
    #useful variables
    more_priority = True
    ...
    if priority_lava == 0 and more_priority:
        ...
        more_priority = handle_portalsLava(game,False,spare_mana)
    ...
    if priority_tornado==1 and more_priority and game.get_my_mana()>=spare_mana:
        debug(game,"need tornado to enemy building")
        more_priority=handel_portalstornado(game,False,spare_mana,priority_portal)
		
    # we loosing
    if priority_Ice == 3 and more_priority and game.get_my_mana() >= spare_mana:
        debug(game, "We loosing - defense")
        more_priority = handle_portalsIce(game,False,spare_mana)
		
    # destroy enemy portal before enemy destroy my
    if priority_tornado==0 and more_priority and game.get_my_mana()>=spare_mana:
        debug(game,"need tornado to enemy building")
        more_priority=handel_portalstornado(game,False,spare_mana,priority_portal)
		
    # we dont need defense and to come enemy castle - we fight
    if priority_tornado==100 and priority_Ice == 100 and priority_portal == 100 and priority_fountain == 100 and more_priority and game.get_my_mana() >= spare_mana and if_enemy_summon_lava:
        debug(game, "There is no something else so - attack ")
        more_priority = handle_portalsLava(game,False,spare_mana)
		
    # we need protect portal or elf  
    if (priority_Ice == 1) and more_priority and game.get_my_mana() >= spare_mana:
        debug(game, "Protect portal or elf")
        more_priority = handle_portalsIce(game,False,spare_mana)
		
    #priority portal 1 we need portal 
    if (priority_portal == 1) and more_priority and "portal" not in without and game.get_my_mana() >= spare_mana:
        debug(game, "Build portal")
        more_priority = need_portal(game,False,mapElves)
        if game.get_my_mana() > portal_mana_to_check_more+spare_mana:
            without.append("portal")
            WhatToCreate(game,mapElves,priority_Ice,priority_portal,priority_fountain,priority_lava,priority_speed_up,priority_invisibility,priority_tornado,without,game.portal_cost+spare_mana)
			
    # we need protect portal or elf  
    if (priority_Ice == 1 or priority_Ice == 2) and more_priority and game.get_my_mana() >= spare_mana:
        debug(game, "Protect portal or elf")
        more_priority = handle_portalsIce(game,False,spare_mana)
		
    # destroy enemy fountain 
    if priority_tornado==2 and more_priority and game.get_my_mana()>=spare_mana:
        debug(game,"need tornado to enemy building")
        more_priority=handel_portalstornado(game,False,spare_mana,priority_portal)
		
    # enemy per turn more my per turn 
    if (priority_fountain == 1) and more_priority and "mana_fountain" not in without and game.get_my_mana() >= spare_mana:
        debug(game,"want to build mama fountain")
        more_priority = need_fountain(game,False,mapElves)
        if game.get_my_mana() > fountain_mana_to_check_more+spare_mana:
            without.append("mana_fountain")
            WhatToCreate(game,mapElves,priority_Ice,priority_portal,priority_fountain,priority_lava,priority_speed_up,priority_invisibility,priority_tornado,without,game.mana_fountain_cost+spare_mana)
			
    # build next to fountain
    if (priority_portal == 2 or priority_portal==3) and more_priority and "portal" not in without and game.get_my_mana() >= spare_mana:
        debug(game, "Build portal")
        more_priority = need_portal(game,False,mapElves)
        if game.get_my_mana() > portal_mana_to_check_more+spare_mana:
            without.append("portal")
    ...
    # same per turn 
    if priority_fountain == 2 and more_priority and "mana_fountain" not in without and game.get_my_mana() >= spare_mana:
        debug(game,"want to build mama fountain")
        more_priority = need_fountain(game,False,mapElves)
        if game.get_my_mana() > fountain_mana_to_check_more+spare_mana:
            without.append("mana_fountain")
    ...
    if priority_lava == 1 and more_priority and game.get_my_mana() >= spare_mana and if_enemy_summon_lava:
        debug(game, "Save elf from ice")
        more_priority = handle_portalsLava(game,False,spare_mana)
		
    # we need help for good fight or to come near to enemy  
    if priority_portal != 100 and more_priority and "portal" not in without and game.get_my_mana() >= spare_mana:
    ...
    if priority_fountain == 3 and more_priority and "mana_fountain" not in without and game.get_my_mana() >= spare_mana:
        debug(game,"want to build mama fountain")
        more_priority = need_fountain(game,False,mapElves)
        if game.get_my_mana() > fountain_mana_to_check_more+spare_mana:
            without.append("mana_fountain")
    ...
    # we can attack  
    if (priority_lava == 2 or priority_lava == 3) and more_priority and game.get_my_mana() >= spare_mana and if_enemy_summon_lava: 
        debug(game, "We can attack")
        more_priority = handle_portalsLava(game,False,spare_mana)
		
    # spare mana - so attack 
    if more_priority and game.get_my_mana() >= spare_mana and if_enemy_summon_lava: 
        debug(game, "We can attack")
        more_priority = handle_portalsLava(game,False,spare_mana)
...
def need_portal(game,for_check,mapElves):
    ...
    myElves = {}
    # check if each elf can build a portal in her Situation
    for elf in game.get_my_living_elves():
        if elf in mapElvesPerTurns.keys() and mapElvesPerTurns[elf] in game.get_enemy_mana_fountains() and len(game.get_enemy_mana_fountains()) > len(game.get_my_mana_fountains()):
            myElves[elf] = False
        nearst_enemy = sorted(game.get_enemy_living_elves() + game.get_enemy_ice_trolls(), key = lambda e: elf.in_attack_range(e))
        myElves[elf] = "Yes"
	...
	# there is'nt enemy elves and enemy elves came before build near castle		
            ...
            if my_portal_to_enemy_castle:
                ...
                if enemy_revaive and enemy_revaive[0].turns_to_revive < turn_elf_to_enemy_castle and elf.distance(game.get_enemy_castle()) < my_portal_to_enemy_castle[0].distance(game.get_enemy_castle()) and elf.distance(game.get_enemy_castle()) <= elf.distance(game.get_my_castle()):
                    if my_portal_to_enemy_castle[0].distance(game.get_enemy_castle()) > game.castle_size + game.portal_size + 3*game.elf_max_speed:
                        if for_check:
                            debug(game, "Save mana - build near his castle before enemy came")
                            return 1
                        if elf.can_build_portal() and myElves[elf] == "Yes":
                            elf.build_portal()
                            debug(game, "Build - build near his castle before enemy came")
                            return True 
        ...
        if 2*elf.distance(game.get_enemy_castle()) < elf.distance(game.get_my_castle()) and ((not (my_portal_to_enemy_castle)) or (elf.distance(game.get_enemy_castle()) < my_portal_to_enemy_castle[0].distance(game.get_enemy_castle()))):
            if elf in mapElvesPerTurns.keys() and mapElvesPerTurns[elf] in game.get_enemy_living_elves() and mapElvesPerTurns[elf] in EnemyElfLoc.keys() and mapElvesPerTurns[elf].distance(game.get_enemy_castle()) < EnemyElfLoc[mapElvesPerTurns[elf]].distance(game.get_enemy_castle()):
                ...
                if ((nearest_enemy != None and enemy_attackers <= attackers_count(game,nearest_enemy)) or (nearest_enemy == None)) and not (enemy_elf_to_elf and enemy_elf_to_elf[0].distance(elf) < enemy_elf_to_elf[0].attack_range + enemy_elf_to_elf[0].max_speed + game.portal_size):
                    debug(game, "Save - build in dangerous place - 1")
                    if for_check:
                        return 1
                    if elf.current_health > 2 and elf.can_build_portal():
                        elf.build_portal()
                        debug(game, "Build - build in dangerous place - 1")
                        return True
						
    # to build in very dangerous place:
    ...
    for elf in my_elf_near_enemy_castle:
        if elf_to_enemy_castle and elf == elf_to_enemy_castle[0] and game.get_enemy_living_elves() and 3*elf.distance(game.get_enemy_castle()) < elf.distance(game.get_my_castle()):
            ...
            if enemy_attackers <= attackers_count(game,enemy) and elf.current_health > game.portal_building_duration and not (myElves[elf]=="No" and game.get_my_mana()<=game.tornado_cost+game.portal_cost)and not (enemy_elf_to_elf and enemy_elf_to_elf[0].distance(elf) < enemy_elf_to_elf[0].attack_range + enemy_elf_to_elf[0].max_speed + game.portal_size):
                debug(game, "Save - build in dangerous place - 2")
                if for_check:
                    return 1
                if elf.current_health > 2 and elf.can_build_portal():
                    elf.build_portal()
                    debug(game, "Build - build in dangerous place - 2")
                    return True

    # to good fight elf to elf - have not portals 
    for elf in game.get_my_living_elves():
        less_mana = 0
        if game.get_my_mana() < (game.portal_cost):
    ...
    portal_enemy_castle = sorted(game.get_my_portals(),key = lambda p: p.distance(game.get_enemy_castle()))
    enemy_portal_near_my_castle = sorted(game.get_enemy_portals(),key = lambda p: p.distance(game.get_my_castle()))
    # there isnt enemy elves and enemy elves came before build near castle
    ...
    if not (game.get_enemy_mana() + game.get_enemy().mana_per_turn *game.portal_building_duration > game.tornado_cost and game.get_my_mana() + game.get_myself().mana_per_turn * game.portal_building_duration < game.portal_cost + game.ice_troll_cost):
        for elf in game.get_my_living_elves():
            ...
            if not (my_portal_to_elf and my_portal_to_elf[0].distance(elf) < 2*game.portal_size + 4*game.tornado_max_speed):
                ...
                enemy_portal_near_this_elf = sorted(game.get_enemy_portals(),key = lambda p: p.distance(elf))
                if enemy_portal_near_this_elf:
                    if enemy_portal_near_this_elf[0].currently_summoning == "IceTroll":
                        turn_to_sum = enemy_portal_near_this_elf[0].turns_to_summon
                        turn_enemy_came = turn_to_sum + (elf.distance(enemy_portal_near_this_elf[0]) - (game.ice_troll_attack_range+1))/game.ice_troll_max_speed
                    else:
                        turn_enemy_came = (elf.distance(enemy_portal_near_this_elf[0])-(2*game.portal_size+1))/elf.max_speed
                    if game.get_myself().mana_per_turn > 0:
                        if game.get_myself().mana_per_turn == 0:
                            turn_to_mana = 0
                        else:
    ...
    my_elf_near_enemy_castle = sorted(game.get_my_living_elves(), key = lambda e: e.distance(EnemyCastle))
    for elf in my_elf_near_enemy_castle:
        enemy_portal_near_my_castle = sorted(game.get_enemy_portals(),key = lambda p: p.distance(MyCastle))
        my_portal_near_enemy_castle = sorted(game.get_enemy_portals(),key = lambda p: p.distance(EnemyCastle))
        if enemy_portal_near_my_castle and not my_portal_near_enemy_castle:
            if elf.distance(EnemyCastle)<elf.distance(MyCastle):
                ...
                if elf.can_build_portal() and myElves[elf] == "Yes":
                    ...
                    return True
            if my_portal_near_enemy_castle:
                if elf.distance(EnemyCastle)<my_portal_near_enemy_castle[0].distance(EnemyCastle):
                    debug(game, "Save mana - near portal than my portals ")
                    if for_check:
                        return 4 
                    if elf.can_build_portal() and myElves[elf] == "Yes":
    ...
    # to build nearst other portal 
    for elf in my_elf_near_enemy_castle:
        near_portal_to_enemy = sorted(game.get_all_portals(), key = lambda p: p.distance(EnemyCastle))
        if near_portal_to_enemy and elf.distance(EnemyCastle) < near_portal_to_enemy[0].distance(EnemyCastle) and near_portal_to_enemy[0].distance(elf) > 2*game.portal_size + 1:
...
def need_fountain(game,for_check,mapElves):
    # useful variables
    enemy_ice_turrgets = sort_enemy_ice(game)
    ...
    for elf in game.get_my_living_elves():
        if game.get_enemy_mana() > 3*game.get_my_mana() and game.get_my_mana() >= game.lava_giant_cost and stay_in_good_place_for_build_a_fountain(game,elf,mapElves):
            if for_check:
    ...
    if not (not if_enemy_destroy_my_mana_fountain and my_mana_per_turn >= 1.5*enemy_mana_per_turn) and ((not game.get_my_living_elves()) or (check_if_elf_close_to_initial)or (len(game.get_my_living_elves())>0 and not game.get_enemy_living_elves()) or (check_all_elves_far_from_initial_loc) ):
    ...
    for elf in game.get_my_living_elves():                
        if elf.can_build_mana_fountain() and good_time_save_mana and stay_in_good_place_for_build_a_fountain(game,elf,mapElves) and check_location_for_build_fountain(game,elf.location):
            debug(game,"Build good time")
            elf.build_mana_fountain()
            return True
			
    #if enemy destroy our fountain 
    if for_check and not ok_to_build_fountain and max_fountains>len(game.get_my_mana_fountains()) and not (sum_enemy_fountain>sum_my_fountain) and not if_it_worth_to_build_mana_fountain(game) :
        return 100
   
    # build enemy is weak
    for elf in game.get_my_living_elves():
        if not game.get_enemy_living_elves() and not game.get_enemy_portals() and elf.distance(game.get_my_castle()) <= elf.distance(game.get_enemy_castle()) and stay_in_good_place_for_build_a_fountain(game,elf,mapElves):
            if for_check:
    ...
    for elf in my_elves_by_distance_to_enemy_castle:
        ...
        if enemy_mana_per_turn ==  my_mana_per_turn + game.mana_fountain_mana_per_turn and (good_portal):
            if for_check:
                return 100
        if ((enemy_mana_per_turn > my_mana_per_turn) or (len(game.get_my_mana_fountains())==0 and not good_portal and not good_build)) and stay_in_good_place_for_build_a_fountain(game,elf,mapElves):
            #if stay_in_good_place_for_build_a_fountain(game,elf,mapElves):
            if for_check :
                debug(game, "Save mana(loosing) - need fountain - 1")
                return 1
            if elf.can_build_mana_fountain() and myElves[elf] == "Yes" and check_location_for_build_fountain(game,elf.location):
                elf.build_mana_fountain()
                debug(game,"Build(loosing) - mana fountain - 1")
                return True
    ...
    if for_check and all_mapElves_enemy_elf and my_portal_to_enemy_castle and enemy_portal_to_my_castle:
        if my_portal_to_enemy_castle[0].distance(game.get_enemy_castle()) < enemy_portal_to_my_castle[0].distance(game.get_my_castle()):
            enemy_elf_to_my_castle = sorted(game.get_enemy_living_elves(), key = lambda e: e.distance(game.get_my_castle()))
            if enemy_elf_to_my_castle:
                my_attackers = sorted(game.get_my_living_elves()+game.get_my_ice_trolls(), key = lambda e: e.distance(enemy_elf_to_my_castle[0]))
                if not(my_attackers and enemy_elf_to_my_castle[0].distance(game.get_my_castle()) < my_portal_to_enemy_castle[0].distance(game.get_my_castle()) and my_attackers[0].distance(enemy_elf_to_my_castle[0]) > 2*my_attackers[0].max_speed):
                    return 100
    ...
    for elf in game.get_my_living_elves():
        if (enemy_fountain_health > my_fountain_health):
            ...
            if stay_in_good_place_for_build_a_fountain(game,elf,mapElves):
                if for_check :
                    debug(game, "Save mana(loosing) - need fountain - 2")
                    return 1
                if elf.can_build_mana_fountain() and myElves[elf] == "Yes" and check_location_for_build_fountain(game,elf.location) and elf.distance(game.get_my_castle()) < game.castle_size + game.mana_fountain_size + 3*elf.max_speed and len(game.get_my_living_elves()) > len(game.get_enemy_living_elves()):
                    elf.build_mana_fountain()
                    debug(game,"Build(loosing) - mana fountain-2")
                    return True 
    ...
    for elf in my_elves_by_distance_to_enemy_castle:
        ...
        if enemy_mana_per_turn == my_mana_per_turn and len(game.get_my_portals()) >= len(game.get_enemy_portals()) and game.get_my_portals() and ok_to_build:
            if stay_in_good_place_for_build_a_fountain(game,elf,mapElves) and not (game.get_enemy_castle().current_health > 1.5*game.get_my_castle().current_health and not game.get_enemy_living_elves()) and not (good_portal != None and not (game.get_enemy_living_elves())):
                if for_check :
                    debug(game, "Save mana(draw) - need fountain ")
                    return 2
                if elf.can_build_mana_fountain() and myElves[elf] == "Yes" and check_location_for_build_fountain(game,elf.location):
                    elf.build_mana_fountain()
                    debug(game,"Build(draw) - mana fountain")
                    return True
    for elf in game.get_my_living_elves():
        if ( (if_enemy_destroy_my_mana_fountain and len(game.get_my_mana_fountains()) <= len(game.get_enemy_mana_fountains())) or (game.get_my_mana() >= game.mana_fountain_cost+game.portal_cost) ) and stay_in_good_place_for_build_a_fountain(game,elf,mapElves):
            if for_check :
                debug(game, "Save mana(loosing/draw - enemy destroy mana) - need fountain ")
                return 2
            if elf.can_build_mana_fountain() and myElves[elf] == "Yes" and check_location_for_build_fountain(game,elf.location):
                elf.build_mana_fountain()
                debug(game, "Build mana(loosing/draw - enemy destroy mana) - need fountain ")
                return True 
    # build behind portal
    for elf in my_elves_by_distance_to_enemy_castle:
        ...
        if optional_locations and elf.location.distance(optional_locations[0]) < game.mana_fountain_size + elf.max_speed:
            if for_check :
                debug(game, "Save mana(build behind portal) - need fountain ")
                return 3
            if elf.can_build_mana_fountain() and myElves[elf] == "Yes" and check_location_for_build_fountain(game,elf.location):
                elf.build_mana_fountain()
                debug(game,"Build(build behind portal) - mana fountain")
                return True
...
def handle_portalsIce(game,for_check,spare_mana):
    # useful lists 
	#list_fountain = global 
    optional_locations = list_fountain
    ...
    if game.get_my_mana() < game.ice_troll_cost:
        EnoughMana = game.ice_troll_cost - game.get_my_mana()
    LavaLoc = {}
    if game.get_myself().mana_per_turn == 0:
        turn_to_ice = TurnsToCreateIce
    else:
        turn_to_ice = (EnoughMana/game.get_myself().mana_per_turn) + TurnsToCreateIce
    ...
    for portal in PortalToEnemyCastle:
        ice_turgget = False
        EnemynElfnearPortal = sorted(game.get_enemy_living_elves(), key = lambda enemy: enemy.distance(portal))
        if EnemynElfnearPortal and spell_casted(game,EnemynElfnearPortal[0]) != "invisibility" and spell_casted(game,EnemynElfnearPortal[0]) != "both of them" and not (game.get_my_castle().current_health < game.castle_max_health/5 and game.get_enemy_castle().current_health > 2*game.get_my_castle().current_health and game.get_enemy_portals()): 
            my_portal_to_enemy_elf = sorted(game.get_my_portals(), key = lambda portal: portal.distance(EnemynElfnearPortal[0]))
            enemy_elf_loc = EnemynElfnearPortal[0].location.towards(my_portal_to_enemy_elf[0],(game.elf_max_speed)*(turn_to_ice))
            ice_attack_elf = True
            ...
            for ice in game.get_my_ice_trolls():
                if my_ice_turrgets[ice] == EnemynElfnearPortal[0] and ice.current_health > 8 and ice not in enemy_ice_turrgets.values():
                    ice_turgget = True
            TurnsOfEnemyElfToMyPortal = 0
            if EnemynElfnearPortal[0].max_speed != 0:
                TurnsOfEnemyElfToMyPortal = (EnemynElfnearPortal[0].distance(portal)-EnemynElfnearPortal[0].attack_range - game.portal_size)/EnemynElfnearPortal[0].max_speed
            if game.get_myself().mana_per_turn == 0:
                turn_to_ice = 0
            else:
                turn_to_ice = (EnoughMana/game.get_myself().mana_per_turn)
            TurnsNeedForPerfectdefense = TurnsToCreateIce+((game.portal_size)/(game.ice_troll_max_speed))+(turn_to_ice)
            if portal == PortalToEnemyCastle[0] and portal.distance(game.get_enemy_castle()) < portal.distance(game.get_my_castle()):
                TurnsNeedForPerfectdefense += TurnsNeedForPerfectdefense/2
            enemy_near_portal = sorted(game.get_enemy_creatures()+game.get_enemy_living_elves() , key = lambda e: e.distance(portal))
            my_ice_to_enemy = [] 
            for ice in game.get_my_ice_trolls():
                if ice.distance(EnemynElfnearPortal[0]) < game.ice_troll_attack_range + game.ice_troll_max_speed and ice not in enemy_ice_turrgets.values():
                    my_ice_to_enemy.append(ice)
            my_ice_to_portal = sorted(game.get_my_ice_trolls(), key = lambda i: i.distance(portal))
            my_elf_to_enemy = sorted(game.get_my_living_elves(), key = lambda e: e.distance(EnemynElfnearPortal[0]))
            enemy_not_danger = False
            portal_will_destroy = False
            if EnemynElfnearPortal[0].in_attack_range(portal) and portal.current_health < game.ice_troll_summoning_duration+1 and attackers_count12(game,EnemynElfnearPortal[0]) == 0:
                portal_will_destroy = True
            if (EnemynElfnearPortal[0].distance(portal) -(EnemynElfnearPortal[0].attack_range + portal.size) )/EnemynElfnearPortal[0].max_speed + portal.current_health/EnemynElfnearPortal[0].attack_multiplier < game.ice_troll_summoning_duration + 1 and attackers_count12(game,EnemynElfnearPortal[0]) == 0:
                portal_will_destroy = True
            enemy_will_die = False
            if EnemynElfnearPortal[0].current_health < game.ice_troll_summoning_duration:
                for ice in game.get_my_ice_trolls():
                    if my_ice_turrgets[ice] == EnemynElfnearPortal[0] and ice.distance(EnemynElfnearPortal[0]) < game.ice_troll_attack_range and enemy_attackers_count(game,ice) <= 1 and ice.current_health > EnemynElfnearPortal[0].current_health and ice not in enemy_ice_turrgets.values():
                        enemy_will_die = True
            for elf in game.get_my_living_elves():
                if attack[elf] == EnemynElfnearPortal[0] and elf.current_health >= EnemynElfnearPortal[0].current_health and EnemynElfnearPortal[0].current_health <= game.ice_troll_summoning_duration +1 and enemy_attackers_count(game,elf) <=1:
                    enemy_will_die = True
            my_elves_to_portal = sorted(game.get_my_living_elves(),key = lambda e: e.distance(portal))
            my_elf_will_save_the_portal = False
            if my_elves_to_portal and my_elves_to_portal[0].distance(portal) < EnemynElfnearPortal[0].distance(portal) + my_elves_to_portal[0].max_speed and my_elves_to_portal[0].distance(EnemynElfnearPortal[0]) < EnemynElfnearPortal[0].distance(portal) + 2*my_elves_to_portal[0].max_speed and my_elves_to_portal[0].current_health >= EnemynElfnearPortal[0].current_health:
                ...
                for enemy_ice in game.get_enemy_ice_trolls():
                    if enemy_ice_turrgets[enemy_ice] == my_elves_to_portal[0] and enemy_ice.distance(my_elves_to_portal[0]) < enemy_ice.attack_range + 5*enemy_ice.max_speed:
                        my_elf_will_save_the_portal = False
            if not (if_enemy_destroy_my_mana_fountain and ((spell_casted(game,EnemynElfnearPortal[0]) == "speed up") or (spell_casted(game,EnemynElfnearPortal[0]) == "both of them"))) and not (EnemynElfnearPortal[0].is_building and EnemynElfnearPortal[0].distance(portal) > game.portal_size + EnemynElfnearPortal[0].attack_range + 4*EnemynElfnearPortal[0].distance(portal)) and not not_equal_to_summing(game,portal) and not my_elf_will_save_the_portal and not enemy_will_die and not portal_will_destroy and not enemy_not_danger and not (my_ice_to_portal and my_ice_to_portal[0].distance(portal) < game.ice_troll_attack_range + game.ice_troll_max_speed and 2*portal.distance(game.get_enemy_castle()) < portal.distance(game.get_my_castle())):
                if len(my_ice_to_enemy) < 2 and TurnsOfEnemyElfToMyPortal <= TurnsNeedForPerfectdefense and portal == my_portal_to_enemy_elf[0] and ice_turgget == False and ice_attack_elf and distance_point_from_line(EnemynElfnearPortal[0].location, EnemyElfLoc[EnemynElfnearPortal[0]], portal.location) < (EnemynElfnearPortal[0].attack_range + EnemynElfnearPortal[0].max_speed + game.portal_size + 1):
                    if for_check:
                        return 1
                    ...
                    if portal.currently_summoning == "IceTroll":
                        return True
                TurnsNeedForPerfectdefense = TurnsToCreateIce+((game.portal_size)/(game.ice_troll_max_speed))
                if len(my_ice_to_enemy) < 2 and TurnsOfEnemyElfToMyPortal < TurnsNeedForPerfectdefense and portal == my_portal_to_enemy_elf[0] and ice_turgget == False and (ice_attack_elf) and not (EnemynElfnearPortal[0] in EnemyElfLoc.keys() and EnemynElfnearPortal[0].distance(portal) > EnemyElfLoc[EnemynElfnearPortal[0]].distance(portal)+EnemynElfnearPortal[0].max_speed/5):
                    if for_check:
                        return 1
                    if not portal.is_summoning and portal.can_summon_ice_troll():
    ...
    b = False
    for portal in game.get_my_portals():
        turns_perfect_defense = ((game.portal_size)/(game.ice_troll_max_speed))+(turn_to_ice)
        for enemy_tornado in game.get_enemy_tornadoes():
            ...
            if turns_tornado_to_portal < (enemy_tornado.current_health/enemy_tornado.suffocation_per_turn - (2+count_ice_attackers*4))and turns_perfect_defense <= turns_tornado_to_portal and enemy_tornado_turrgets[enemy_tornado] == portal:
                if for_check:
                    return 1
                if not portal.is_summoning and portal.can_summon_ice_troll():
    ...
    for portal in game.get_my_portals():
        ...
        if my_fountain and my_fountain[0].distance(portal) < 1500:
            ...
            if len(game.get_enemy_living_elves()) > len(game.get_my_living_elves()):
                spare_distance = 4
            if enemy_elves_to_portal and enemy_elves_to_portal[0].distance(portal) < (2*enemy_elves_to_portal[0].max_speed +enemy_elves_to_portal[0].attack_range)*spare_distance + game.mana_fountain_size and enemy_elves_to_portal[0] and enemy_elves_to_portal in EnemyElfLoc.keys() and EnemyElfLoc[enemy_elves_to_portal[0]].distance(portal) > enemy_elves_to_portal[0].distance(portal) + 2*enemy_elves_to_portal[0].max_speed/3:
                need_ice = True
            if if_enemy_destroy_my_mana_fountain and enemy_elves_to_portal and enemy_elves_to_portal[0] in EnemyElfLoc.keys() and EnemyElfLoc[enemy_elves_to_portal[0]].distance(portal) > enemy_elves_to_portal[0].distance(portal) and ((spell_casted(game,enemy_elves_to_portal[0]) == "speed up" or spell_casted(game,enemy_elves_to_portal[0]) == "both of them") or (enemy_elves_to_portal[0].distance(portal) < enemy_elves_to_portal[0].max_speed*15)):
                need_ice = True
            if enemy_elves_to_portal and ((spell_casted(game,enemy_elves_to_portal[0])== "speed up") or (spell_casted(game,enemy_elves_to_portal[0]) == "both of them")) and enemy_elves_to_portal[0].distance(portal) < (4*enemy_elves_to_portal[0].max_speed +enemy_elves_to_portal[0].attack_range)*spare_distance + game.mana_fountain_size:
                need_ice = True
    ...
    for volcano in game.get_active_volcanoes(): 
        if not (game.get_my_living_elves())  and volcano_life[volcano] > volcano.current_health and volcano.damage_by_me < volcano.damage_by_enemy + volcano.current_health and volcano.current_health < volcano.max_health/2:
    ...
    # we have not elves and dangerous elf :
    if not game.get_my_living_elves() and game.get_enemy_living_elves():
        ...
        enemy_elf_to_my_castle = sorted(game.get_enemy_living_elves(), key = lambda e: e.distance(game.get_my_castle()))
        portal_to_enemy = sorted(game.get_my_portals(),key = lambda p: p.distance(enemy_elf_to_my_castle[0]) + p.distance(game.get_my_castle()))
        if portal_to_enemy and (enemy_elf_to_my_castle[0].distance(game.get_my_castle()) - build_dis)< 2*(enemy_elf_to_my_castle[0].distance(game.get_enemy_castle()) - build_dis):
            enemy_elf_loc = enemy_elf_to_my_castle[0].location.towards(portal_to_enemy[0],(game.elf_max_speed)*(turn_to_ice))
            ice_attack_elf = True
            for k in LavaLoc.keys():
                if LavaLoc[k].distance(portal) <= enemy_elf_loc.distance(portal)+3*game.elf_max_speed and k.current_health > game.ice_troll_summoning_duration:
                    ice_attack_elf = False
                if ice_attack_elf:
                    ...
                    if not portal_to_enemy[0].is_summoning and portal_to_enemy[0].can_summon_ice_troll() and ice_attack_elf:
                        debug(game, "we havnt elves and dangerous elf - summon ice")
    ...
    for portal in game.get_my_portals():
        ...
        EnemynnearPortal = sorted(game.get_enemy_living_elves(), key = lambda enemy: enemy.distance(portal))
        ...
        if not not_equal_to_summing(game,portal) and EnemynnearPortal and MyElfsfFromMyPortal and not enemy_go_my_fountain and not (EnemynnearPortal[0].current_health < MyElfsfFromMyPortal[0].current_health and portal.current_health <= 2 and enemy_attackers_count(game,MyElfsfFromMyPortal[0]) <=1):
            turn_enemy_can_attack_my_elf = (EnemynnearPortal[0].distance(MyElfsfFromMyPortal[0])-EnemynnearPortal[0].attack_range)/EnemynnearPortal[0].max_speed
            if game.get_myself().mana_per_turn == 0:
                turn_to_ice = 0
            else:
                turn_to_ice = (EnoughMana/game.get_myself().mana_per_turn)
            ...
            if turn_enemy_can_attack_my_elf <= turn_portal_can_save_me and MyElfsfFromMyPortal[0].distance(portal)<5*MyElfsfFromMyPortal[0].max_speed*optional_number:
                ...
                enemy_near_portal = sorted(game.get_enemy_creatures()+game.get_enemy_living_elves() , key = lambda e: e.distance(portal))
                enemy_elf_loc = EnemynElfnearPortal[0].location.towards(PortalToEnemy[0],(game.elf_max_speed)*(turn_to_ice))
                ice_attack_elf = True
                for k in LavaLoc.keys():
                    if LavaLoc[k].distance(portal) <= enemy_elf_loc.distance(portal)+3*game.elf_max_speed and k.current_health > game.ice_troll_summoning_duration:
                        ice_attack_elf = False
                if enemy_attackers_count(game,MyElfsfFromMyPortal[0]) == 1 and MyElfsfFromMyPortal[0].current_health >= EnemynElfnearPortal[0].current_health and attack[MyElfsfFromMyPortal[0]] == EnemynElfnearPortal[0] and MyElfsfFromMyPortal[0].current_health < game.tornado_summoning_duration + 1 and MyElfsfFromMyPortal[0] not in enemy_ice_turrgets.values():
                    not_need_ice = True
                if not_need_ice and portal == PortalToEnemy[0] and (ice_attack_elf) and not (portal.distance(game.get_enemy_castle()) < game.castle_size + game.portal_size +  3*game.elf_max_speed and spell_casted(game,EnemynElfnearPortal[0]) == "speed up" and max_fountains > len(game.get_my_mana_fountains())):
                    if for_check:
                        return 2
                    if not portal.is_summoning and portal.can_summon_ice_troll():
    ...
    if PortalToMyCastle and attackers:
        ...
        # I have not elves and enemy elves
        if not game.get_my_living_elves() and game.get_enemy_living_elves():
            danger_enemy = False
            for enemy in game.get_enemy_living_elves():
        ...
        if health_attackers_on_my_castle >= 25 and my_ice_attack == False:
            ...
            return ice_troll_full_summoning(game,portal_for_protect,spare_mana,message)
...
def handle_portalsLava(game,for_check,spare_mana):
    # useful variables 
    TurnsToCreateLava = game.lava_giant_summoning_duration
    EnoughMana = 0
    if game.get_my_mana() < game.lava_giant_cost:
        EnoughMana = game.lava_giant_cost - game.get_my_mana()
    PortalToEnemyCastle = sorted(game.get_my_portals(), key = lambda portal: portal.distance(game.get_enemy_castle()))
    EnemyPortalToMyCastle = sorted(game.get_enemy_portals(), key = lambda portal: portal.distance(game.get_my_castle()))
    ...
    if len(game.get_enemy_living_elves()) > len(game.get_my_living_elves()) and if_enemy_destroy_my_fountain and not game.get_enemy_portals() and game.get_my_mana() < game.mana_fountain_cost + game.lava_giant_cost:
        ...
        if ((not my_by_health) or (enemy_by_health and my_by_health[0].current_health < enemy_by_health[0].current_health)): 
            ...
            return True
    # we gona win.....
    if PortalToEnemyCastle and not not_worth_to_summing_tornado(game,PortalToEnemyCastle[0]): 
    ...
    for portal in game.get_my_portals():
        MyElfsfFromMyPortal = sorted(game.get_my_living_elves(), key = lambda MyElf: MyElf.distance(portal))
        EnemynnearPortal = sorted(game.get_enemy_ice_trolls(), key = lambda enemy: enemy.distance(portal))
        if EnemynnearPortal and MyElfsfFromMyPortal and not not_equal_to_summing(game,portal):
            turn_enemy_can_attack_my_elf = (EnemynnearPortal[0].distance(MyElfsfFromMyPortal[0])-EnemynnearPortal[0].attack_range)/EnemynnearPortal[0].max_speed
            if game.get_myself().mana_per_turn == 0:
                turn_to_ice = 0
            else:
            ...
            if turn_enemy_can_attack_my_elf <= turn_portal_can_save_me and (portal.distance(MyElfsfFromMyPortal[0])/game.ice_troll_max_speed+game.ice_troll_summoning_duration) < EnemynnearPortal[0].current_health:
                PortalToEnemy = sorted(game.get_my_portals(), key = lambda p: p.distance(EnemynnearPortal[0]))
                distance_enemy_to_enemy_castle = EnemynnearPortal[0].distance(game.get_enemy_castle())
                distance_portal_to_enemy_castle = PortalToEnemy[0].distance(game.get_enemy_castle())
                if not not_equal_to_summing(game,portal) and portal == PortalToEnemy[0] and distance_enemy_to_enemy_castle < distance_portal_to_enemy_castle and EnemynnearPortal[0].current_health > game.lava_giant_summoning_duration:
                    if for_check:
                        return 1
                    ...
                    if portal.currently_summoning == "LavaGiant":
                        return True
    # check if need the mana for defense
    ...
    # my portal near enemy castle then enemy portal near my castle - (we can't loss)
    PortalToEnemyCastle = sorted(game.get_my_portals(), key = lambda portal: portal.distance(game.get_enemy_castle()))
    EnemyPortalToMyCastle = sorted(game.get_enemy_portals(), key = lambda portal: portal.distance(game.get_my_castle()))
    b = False 
    ...
    if not enemy_in_dangerous_place and PortalToEnemyCastle:
        if for_check:
            return 100
        for portal in PortalToEnemyCastle:
            if ((portal_need_over_protect(game,portal) == False) or (game.get_my_mana() >= game.lava_giant_cost + game.ice_troll_cost)) and not not_equal_to_summing(game,portal):
                if game.get_my_mana() >= game.lava_giant_cost + game.ice_troll_cost:
                    if not portal.is_summoning and portal.can_summon_lava_giant() and game.get_my_mana() - (spare_mana+game.lava_giant_cost) > 0 and portal.distance(game.get_enemy_castle()) < 2*portal.distance(game.get_my_castle()):
                        debug(game, "special event - summon lava giant")
                        portal.summon_lava_giant()
                        b = True
                    elif not portal.is_summoning and portal.can_summon_lava_giant() and game.get_my_mana() - (spare_mana+game.lava_giant_cost + game.mana_fountain_cost) > 0 and game.lava_giant_suffocation_per_turn*(int((portal.distance(game.get_my_castle())-game.lava_giant_attack_range)/game.lava_giant_max_speed)+5/game.lava_giant_attack_multiplier)<=game.lava_giant_max_health:
                        debug(game, "special event (all the attackers portals)- summon lava giant")
                        portal.summon_lava_giant()
                        b = True
                    elif not portal.is_summoning and portal.can_summon_lava_giant() and game.get_my_mana() > spare_mana + (game.mana_fountain_cost + game.portal_cost)/2 + game.lava_giant_cost:
...
def handel_portalstornado(game,for_check,spare_mana,priority_portal):
    ...
    for portal in game.get_my_portals():
        ...
        if building_to_portal and not not_worth_to_summing_tornado(game,portal):
            count1=0
            count2=0
                ...
                if portal_to_building and portal==portal_to_building[0] and building_to_portal[0] in game.get_enemy_portals():
                    ...
                    if  check2 and check3 and check7 and not check_if_someone_attack and  enemy_portal_to_me and  enemy_portal_to_me[0].distance(portal)<diagonal_line/3.5 and ((check) or (enemy_elf_to_portal and enemy_elf_to_portal[0].distance(portal)<enemy_elf_to_portal[0].max_speed+enemy_elf_to_portal[0].attack_range)):
                        ...
                        if not portal.is_summoning and portal.can_summon_tornado():
                            debug(game, "there is portal next to me -summon tornado-0")
                    ...
                    if check and check2 and if_dangerous_portal and if_portal_far_from_middle and not check_if_someone_attack:
                        if for_check :
                            return 0
                        if not portal.is_summoning and portal.can_summon_tornado():
                            debug(game, "there is ahushiling dangerous portal-summon tornado-0")
                            portal.summon_tornado()
                            d=True
                        if portal.currently_summoning == "Tornado":
                            d=True
                    if building_to_portal[0].distance(portal)<diagonal_line/18+game.portal_size*2 and check and check2 and not check_if_someone_attack:
                        if for_check :
                            return 0
                        if not portal.is_summoning and portal.can_summon_tornado():
                            debug(game, "there is ahushiling close portal-summon tornado-0")
                            portal.summon_tornado()
                            d=True
                        if portal.currently_summoning == "Tornado":
                            d=True
    ...
    #there is enemy portal next to me (very near )- we summon
    d=False
    for portal in game.get_my_portals():
        enemy_portal_to_portal=sorted(game.get_enemy_portals(),key=lambda p:p.distance(portal))
        if not not_worth_to_summing_tornado(game,portal) and enemy_portal_to_portal and enemy_portal_to_portal[0].distance(portal)<game.portal_size*3+game.tornado_max_speed*game.tornado_summoning_duration+300:
            ...
            if check and check7 and not check_if_someone_attack and portal_to_building and portal_to_building[0]==portal :
                ...
                if portal_summing:
                    if for_check :
                        return 1
                    if not portal.is_summoning and portal.can_summon_tornado():
                        debug(game, "there is enemy portal next to me -summon tornado-1")
                        portal.summon_tornado()
                        d=True
                    if portal.currently_summoning == "Tornado":
                        d=True
    ...
    for enemy_elf in game.get_enemy_living_elves():
        ...
        if (enemy_elf.currently_building =="Portal" or (enemy_elf.currently_building =="ManaFountain" and not game.get_enemy_portals())) and enemy_elf.turns_to_build<=game.tornado_summoning_duration and ( (portal_to_enemy_elf and  (portal_to_enemy_elf[0].distance(enemy_elf)<(2*game.portal_size +game.tornado_attack_range + game.tornado_max_speed*game.tornado_max_health/3*game.tornado_suffocation_per_turn)) )or (not game.get_enemy_portals() and not game.get_enemy_ice_trolls() and portal_to_enemy_elf and (portal_to_enemy_elf[0].distance(enemy_elf)-(game.mana_fountain_size+game.tornado_attack_range))/game.tornado_max_speed < (3*game.tornado_max_health/4 )) or (not game.get_enemy_portals() and portal_to_enemy_elf and (portal_to_enemy_elf[0].distance(enemy_elf)-(game.mana_fountain_size+game.tornado_attack_range))/game.tornado_max_speed < (3*game.tornado_max_health/4 - 3 )) ):
            ...
            if enemy_succes_build and not other_tornado_attack and not not_worth_to_summing_tornado(game,portal_to_enemy_elf[0]) and check3 and ((not building_to_portal)or (building_to_portal and building_to_portal[0].distance(portal_to_enemy_elf[0])>enemy_elf.distance(portal_to_enemy_elf[0]))):
                if for_check :
                    return 1
                if not portal_to_enemy_elf[0].is_summoning and portal_to_enemy_elf[0].can_summon_tornado():
                    debug(game, "there is enemy elf next to me that build a portal - summon tornado-1")
    ...
    # dangerous portal next to me so summon tornado
    portal_to_my_castle = sorted(game.get_my_portals() , key = lambda p: p.distance(game.get_my_castle()))
                ...
                if (building_to_portal[0] in dangerous_portal1.values() or if_dangerous_portal)and portal_to_building and portal==portal_to_building[0] :
                    ...
                    if check2 and check7 and check and not check_if_someone_attack and portal_to_building and portal_to_building[0] == portal :
                        if for_check :
                            return 1
                        if not portal.is_summoning and portal.can_summon_tornado() :
                            debug(game, "there is dangerous enemy building that we can destroy -summon tornado-1")
                            portal.summon_tornado()
                            d=True
                        if portal.currently_summoning == "Tornado":
                            d= True
                ...
                if portal_to_building and portal==portal_to_building[0] and building_to_portal[0] in game.get_enemy_mana_fountains():
                    ...
                    if not check_if_elf_already_go and check2 and check and not check_if_someone_attack:
                        if for_check :
                            return 2
                        if not portal.is_summoning and portal.can_summon_tornado() :
                            debug(game, "there is enemy mana fountain that we can destroy (priority 2)- summon tornado ")
                            portal.summon_tornado()
                            d=True
                        if portal.currently_summoning == "Tornado":
                            d= True
    ...
    #we are control the game so we summon tornado more often     
    for portal in game.get_my_portals():
        building_to_portal=sorted(game.get_enemy_mana_fountains()+game.get_enemy_portals(),key=lambda b:b.distance(portal))
        if building_to_portal and not not_worth_to_summing_tornado(game,portal):
            count1=0
    ...
    if not game.get_enemy_portals() and game.get_my_portals() and game.get_enemy_mana_fountains() and len(game.get_enemy_mana_fountains()) >= len(game.get_my_mana_fountains()) and not game.get_enemy_ice_trolls():
        ...
        for elf1 in game.get_my_living_elves():
            if elf1 in mapElvesPerTurns.keys() and mapElvesPerTurns[elf1] == building_to_portal[0] and elf1.distance(mapElvesPerTurns[elf1])<elf1.attack_range+building_to_portal[0].size+diagonal_line/12:
                check_if_elf_already_go=True
        if not check_if_elf_already_go and not not_worth_to_summing_tornado(game,portal) and enemy_fountain not in attack.values() and (portals_dis/game.tornado_max_speed)*game.tornado_suffocation_per_turn < 2*(game.tornado_max_health)/3 - (enemy_fountain.current_health/game.tornado_attack_multiplier) and check:
            if for_check :
                return 2
...
def tornado_turget_after_summing(game,portal):
	#return the destination of tornado if i will summon from "portal"
...
def ice_troll_full_summoning(game,portal_for_protect,spare_mana,message):
    b = False
    if not not_equal_to_summing(game,portal_for_protect[0]) and not portal_for_protect[0].is_summoning and portal_for_protect[0].can_summon_ice_troll() and game.get_my_mana() > spare_mana:
...
def stay_in_good_place_for_build_a_fountain(game,elf,mapElves):
	#check if it worth to the elf to start save mana for fountain -its check if elf will stay in good place to build mana fountain
...
def portal_need_over_protect(game,portal):
...
def check_location_for_build_fountain(game,loc):
	#check if it good to build fountain in "loc"
...
def check_location_for_build_portal(game,loc):
	#check if it good to build portal in "loc"
    enemy_portal_to_loc = sorted (game.get_enemy_portals(),key=lambda p:p.distance(loc))
...
def not_equal_to_summing(game,portal):
	#check if enemy attack my portal and i will not finish summon what i want 
    EnemynElfnearPortal = sorted(game.get_enemy_living_elves() ,key = lambda e: e.distance(portal))
    enemy_tornado = sorted(game.get_enemy_tornadoes() ,key = lambda e: e.distance(portal))
    portal_will_destroy = False
    if EnemynElfnearPortal and (EnemynElfnearPortal[0].distance(portal) -(EnemynElfnearPortal[0].attack_range + portal.size) )/EnemynElfnearPortal[0].max_speed + portal.current_health/EnemynElfnearPortal[0].attack_multiplier < game.ice_troll_summoning_duration + 1 and attackers_count12(game,EnemynElfnearPortal[0]) == 0:
        portal_will_destroy = True
...
def not_worth_to_summing_tornado(game,portal):
    EnemynElfnearPortal1 = sorted(game.get_enemy_tornadoes() ,key = lambda e: e.distance(portal))
    portal_will_destroy = False
    if EnemynElfnearPortal1 and (EnemynElfnearPortal1[0].distance(portal) -(EnemynElfnearPortal1[0].attack_range + portal.size) )/EnemynElfnearPortal1[0].max_speed + portal.current_health/EnemynElfnearPortal1[0].attack_multiplier < game.tornado_summoning_duration + 1 and (attackers_count12(game,EnemynElfnearPortal1[0]) - elf_attackers_count(game,EnemynElfnearPortal1[0])) <= 0 and EnemynElfnearPortal1[0].current_health >= (EnemynElfnearPortal1[0].distance(portal) -(EnemynElfnearPortal1[0].attack_range + portal.size) )/EnemynElfnearPortal1[0].max_speed + portal.current_health/EnemynElfnearPortal1[0].attack_multiplier:
        portal_will_destroy = True
    if EnemynElfnearPortal1 and EnemynElfnearPortal1[0].distance(portal) < portal.size + 2*game.tornado_max_speed + game.tornado_attack_range and portal.current_health < 3:
        portal_will_destroy = True
    EnemynElfnearPortal = sorted(game.get_enemy_living_elves() ,key = lambda e: e.distance(portal))
    if EnemynElfnearPortal and (EnemynElfnearPortal[0].distance(portal) -(EnemynElfnearPortal[0].attack_range + portal.size) )/EnemynElfnearPortal[0].max_speed + portal.current_health/EnemynElfnearPortal[0].attack_multiplier < game.tornado_summoning_duration + 1 and attackers_count12(game,EnemynElfnearPortal[0]) <= 0:
        portal_will_destroy = True
    
    return portal_will_destroy
    
def if_it_worth_to_build_mana_fountain(game):
	#return True - if we need mana fountain -if enemy destroy my fountain a lot  
        ...
        if game.get_my_portals() and not enemy_in_dangerous_place and mapElves[elf] in game.get_enemy_mana_fountains() and elf.current_health >= mapElves[elf].current_health and (elf.distance(mapElves[elf]) -(elf.attack_range + game.mana_fountain_size))> 1.5*elf.max_speed*game.speed_up_multiplier and casting_elves[elf]==False and (spell_casted(game,elf) != "both of them" and  spell_casted(game,elf) !="speed up") and ((attack[elf]==None) or (enemy_can_attack and not elf.distance(enemy_can_attack[0]) < enemy_can_attack[0].attack_range + enemy_can_attack[0].max_speed+1)) and( (not game.get_enemy_living_elves()) or (len(game.get_enemy_mana_fountains())>len(game.get_my_mana_fountains()) ) or enemy_go_to_my_fountain_naw ) :
            ...
            if check3 and not (elves_to_fountain and elves_to_fountain[0].in_attack_range(mapElves[elf])) and elf == elves_to_fountain[0] and not check_if_portal_next_to_fountain:
                if for_check:
                    return 1
                if elf.can_cast_speed_up():
                    sortdebug(game,"speed up to destroy enemy fountain")
                    elf.cast_speed_up()
                    casting_elves[elf]=True
                    return True
...
def SortElves(game,priority_fountain,priority_portal):
    # Global variables
    global mapElvesPerTurns
    global danger_elves1
    ...
    optional_dangerous_portals = sorted(game.get_enemy_portals(), key=lambda p: p.distance(game.get_my_castle()))
    optional_dangerous_portals = filter(lambda p: (p.distance(game.get_my_castle()) - (game.lava_giant_attack_range + game.castle_size)) > game.lava_giant_max_health / 3 and p.is_summoning,optional_dangerous_portals)
    ...
    for elf in game.get_my_living_elves():
        ...
        if not elf_in_use and len(game.get_enemy_ice_trolls())>40 and len(game.get_all_my_elves())==1:
            sortdebug(game,"Go to enemy castle-1")
            mapElves[elf] = game.get_enemy_castle()
        ...
        if game.get_my_portals() and not check_if_someone_go and enemy_fountain_to_me and elf in mapElvesPerTurns.keys() and mapElvesPerTurns[elf] in game.get_enemy_mana_fountains() and not need_elf(game,mapElvesPerTurns,enemy_fountain_to_me[0],elf):
            sortdebug(game,"stay against fountains")
        ...
        #danger elves locations before Disappearance
        ...
        # to destroy his castle before he will came 
        if not elf_in_use and elf.in_attack_range(game.get_enemy_castle()) and len(game.get_enemy_creatures()) == 0 and len(game.get_enemy_portals()) == 0:
            ...
            if enemy_elves_revive and enemy_elves_revive[0].turns_to_revive + game.portal_building_duration >= game.get_enemy_castle().current_health/elf.attack_multiplier:
                sortdebug(game,"Go to enemy castle before enemy came ")
                mapElves[elf] = game.get_enemy_castle()
                mapElvesPerTurns[elf] = mapElves[elf]
                elf_in_use = True
                
        #go to destroy dangerous fountain against week2 bots       
        ...
        # go to volcano         
        if not elf_in_use and when_I_go_to_volcano(game,elf)==1 and my_volcano[elf]!=None:
            sortdebug(game,"go to attack volcano")
            mapElves[elf] = my_volcano[elf]
            mapElvesPerTurns[elf] = mapElves[elf]
            elf_in_use = True
            
        # there is only dangerous_elf 
        if not elf_in_use and dangerous_elf1[elf]!= None:
            enemy_portal_to_my_castle = sorted(game.get_enemy_portals(), key = lambda p: p.distance(MyCastle))
            if enemy_portal_to_my_castle and enemy_portal_to_my_castle[0].distance(MyCastle) < dangerous_elf1[elf].distance(MyCastle)and not need_elf(game,mapElvesPerTurns,enemy_portal_to_my_castle[0],elf) and enemy_portal_to_my_castle[0].distance(game.get_my_castle()) < 1.5*enemy_portal_to_my_castle[0].distance(game.get_enemy_castle()):
                sortdebug(game, "Going to portal near castle")
                mapElves[elf] = enemy_portal_to_my_castle[0]
                mapElvesPerTurns[elf] = mapElves[elf]
                elf_in_use = True
        ...
        #destroy mana fountains        
        if not elf_in_use and max_fountains > 0 and not game.get_my_mana_fountains() and game.get_enemy().mana_per_turn >=game.get_myself().mana_per_turn and if_enemy_destroy_my_mana_fountain and not check_if_there_is_better_elf_for_chase(game,elf):
            enemy_fountain_to_me = sorted(game.get_enemy_mana_fountains(), key = lambda f: f.distance(elf))
        ...
        #Go to build fountain in good place in front of portal
        check=True
        if not if_enemy_destroy_my_mana_fountain and not elf_in_use and len(game.get_enemy_mana_fountains()) > len(game.get_my_mana_fountains()) and not game.get_enemy_living_elves(): 
            if game.get_my_mana() >= 1.5*game.mana_fountain_cost*elves_go_build_fountain and (priority_fountain == 1) and good_places_for_fountain and not need_elf(game,mapElvesPerTurns,good_places_for_fountain[0],elf):
                ...
                if elf_to_loc and elf==elf_to_loc[0] and check and game.get_enemy().mana_per_turn > 2*game.get_myself().mana_per_turn:
                    if not (len(game.get_enemy_living_elves()) > len(game.get_my_living_elves()) and len(game.get_my_mana_fountains()) < max_fountains):    
                        sortdebug(game,"Go to place for building fountain behind portal - priority_fountain 1")
                        mapElves[elf] = good_places_for_fountain[0]
                        mapElvesPerTurns[elf] = mapElves[elf]
                        elves_go_build_fountain +=1
                        elf_in_use = True
        
		#Go to build portal in good place in front of fountain
        check4=True
        ...
        # go to volcano         
        if not elf_in_use and when_I_go_to_volcano(game,elf) and my_volcano[elf]!=None:
            sortdebug(game,"go to attack volcano")
            mapElves[elf] = my_volcano[elf]
            mapElvesPerTurns[elf] = mapElves[elf]
            elf_in_use = True
        ...
        # go to build portal in enemy fountain places :
        if not elf_in_use:
            best_enemy_loc = None
            portal_in_loc = False
            for portal in game.get_my_portals():
        ...
        # all spell and there isnt dangerous elf or portal - go enemy castle:
        if not elf_in_use and spell_casted(game,elf) == "both of them":
            ...
            if not (elf.distance(loc_to_build_portal) < game.portal_size+1 and elf.can_build_portal() == False and game.get_my_mana() >= game.portal_cost) and not (len(game.get_all_my_elves())<len(game.get_all_enemy_elves()) and my_portal_to_enemy_castle and my_portal_to_enemy_castle[0].distance(game.get_enemy_castle()) <game.castle_size+game.portal_size+diagonal_line/7) and game.portal_cost < 2000:
                sortdebug(game,"Go to enemy castle for build a portal")
                mapElves[elf] = loc_to_build_portal
            else:
                sortdebug(game,"Go to enemy castle-2")
                mapElves[elf] = game.get_enemy_castle()
            mapElvesPerTurns[elf] = mapElves[elf]
            elf_in_use = True
        ...
        # all enemy elf more than my against week 2 bots 
        ...
        if elf in mapElves.keys() and (mapElves[elf] in game.get_enemy_living_elves() or mapElves[elf] in game.get_enemy_portals()):
            ...
            if elf.current_health == 1 and mapElves[elf].current_health > 3*elf.current_health and my_elves_to_turget[0] != elf:
                sortdebug(game,"destination has been changed")
                elf_in_use = False
        
        # there isn't enemy elves - elf have max_speed go to enemy castle
        if not elf_in_use and not game.get_enemy_living_elves() and len(game.get_my_mana_fountains()) >= len(game.get_enemy_mana_fountains()):
            if (spell_casted(game,elf) == "both of them" or spell_casted(game,elf) == "speed up"):
                ...
                sortdebug(game,"Go to enemy castle for build a portal - no enemy elves")
                mapElves[elf] = loc_to_build_portal
                mapElvesPerTurns[elf] = mapElves[elf]
                elf_in_use = True
        ...
        #go to enemy fountain - have nothing to do
        if not elf_in_use and len(game.get_enemy_mana_fountains()) >= len(game.get_my_mana_fountains()) and len(game.get_my_mana_fountains()) > 0:
            ...
            if enemy_fountain_to_me and not mana_in_values and not need_elf(game,mapElvesPerTurns,enemy_fountain_to_me[0],elf) :
                sortdebug(game,"go to enemy fountain - have nothing to do")
                mapElves[elf] = enemy_fountain_to_me[0]
                mapElvesPerTurns[elf] = mapElves[elf]
                elf_in_use = True
                
        # fight against dangerous elf if elf have nothing to do
        if not elf_in_use:
        ...
        if not elf_in_use and optional_dangerous_portals:
            ...
            if optional_enemy_to_attack_portals and optional_enemy_to_attack_portals[0].distance(elf) < optional_dangerous_portals[0].distance(elf) and optional_enemy_to_attack_portals[0] not in dangerous_elf1.values() and optional_enemy_to_attack_portals[0].distance(elf) < 4*elf.max_speed + elf.attack_range and optional_enemy_to_attack_portals[0].distance(elf) < 4*elf.max_speed + elf.attack_range:
                ...
                other_tornado_attack = False
                for elf1 in game.get_my_living_elves():
                    if elf1.in_attack_range(optional_dangerous_portals[0]) and elf1 != elf and elf.distance(optional_dangerous_portals[0]) > 6*elf.max_speed:
                        other_elf_attack = True
                for tornado in game.get_my_tornadoes():
                    if my_tornado_turrgets[tornado] == optional_dangerous_portals[0]:
                        other_tornado_attack = True
                if not other_elf_attack and not other_tornado_attack:
                    mapElves[elf] = optional_dangerous_portals[0]
                    mapElvesPerTurns[elf] = mapElves[elf]
                    elves_go_build_fountain +=1
        ...
        check=True
        if not elf_in_use and game.get_my_mana() >= 1.5*game.mana_fountain_cost*elves_go_build_fountain and ((len(game.get_enemy_living_elves()) < len(game.get_my_living_elves())) or (2*elf.distance(game.get_my_castle()) < elf.distance(game.get_enemy_castle()))) and ((priority_fountain == 1) or (priority_fountain == 2 and priority_portal != 2 and priority_portal != 1) or (priority_fountain == 3 and priority_portal == 100)) and good_places_for_fountain and ( (len(game.get_enemy_living_elves())==0 and elf.distance(good_places_for_fountain[0])<5*elf.max_speed) or (game.get_enemy_living_elves() and elf.distance(good_places_for_fountain[0])<diagonal_line/7) )  and game.get_myself().mana_per_turn<=game.get_enemy().mana_per_turn and not need_elf(game,mapElvesPerTurns,good_places_for_fountain[0],elf):
            elf_to_loc=sorted(game.get_my_living_elves(),key=lambda e:e.distance(good_places_for_fountain[0]))
            for elf1 in mapElvesPerTurns.keys():
                if elf1!=elf and mapElvesPerTurns[elf1]!=None and mapElvesPerTurns[elf1] not in game.get_all_enemy_elves() and mapElvesPerTurns[elf1].distance(good_places_for_fountain[0])<300 and elf1.distance(mapElvesPerTurns[elf1])<elf.distance(good_places_for_fountain[0]):
                    check=False
                ...
                if enemy_fountain_to_elf and ((len(game.get_my_living_elves()) > len(game.get_enemy_living_elves())) or (elf.distance(enemy_fountain_to_elf[0]) < elf.attack_range + 5*elf.max_speed + game.mana_fountain_size)):
                    ...
                    if my_health >= enemy_health and elf.distance(game.get_enemy_castle()) < elf.distance(game.get_my_castle()) and better_elf_for_destroy_fountain(game,elf,enemy_fountain_to_me[0])==elf and not other_elf_go:
                        ...
                        if my_portal_to_enemy_castle and not enemy_portal_to_my_castle and not need_elf(game,mapElvesPerTurns,enemy_fountain_to_elf[0],elf):
                            sortdebug(game,"Destroy enemy fountain - 1")
                        ...
                        if my_portal_to_enemy_castle and enemy_portal_to_my_castle and my_portal_to_enemy_castle[0].distance(game.get_enemy_castle()) < enemy_portal_to_my_castle[0].distance(game.get_my_castle()) and not need_elf(game,mapElvesPerTurns,enemy_fountain_to_elf[0],elf):
                            sortdebug(game,"Destroy enemy fountain - 2")
                            mapElves[elf] = enemy_fountain_to_elf[0]
                    ...
                    if  count1<2 and not need_elf(game,mapElvesPerTurns,enemy_portal,elf) and elf not in mapElves.keys() and enemy_portal.current_health<game.elf_attack_multiplier*30 and not (len(game.get_all_my_elves())<len(game.get_all_enemy_elves()) and enemy_portal.distance(elf)>20*elf.max_speed):
                        if optional_enemy_to_attack_portals and optional_enemy_to_attack_portals[0].distance(elf) < enemy_portal.distance(elf):
                            ...
                            if elf not in mapElves.keys():
                                ...
                                if game.can_build_mana_fountain_at(loc_to_build_fountain) and priority_fountain == 1 and elf.distance(loc_to_build_fountain) < 4*elf.max_speed and game.get_my_mana() + (elf.distance(loc_to_build_fountain)/elf.max_speed)*game.get_myself().mana_per_turn >= game.mana_fountain_cost:
                ...
                if elf not in mapElves.keys() and not (elf.distance(loc_to_build_portal) < game.portal_size+1 and elf.can_build_portal() == True and game.get_my_mana() >= game.portal_cost) and not (len(game.get_all_my_elves())<len(game.get_all_enemy_elves()) and my_portal_to_enemy_castle and my_portal_to_enemy_castle[0].distance(game.get_enemy_castle()) <game.castle_size+game.portal_size+diagonal_line/7) and game.portal_cost < 2000:
                    ...
                    if (not portal_in_loc and (game.get_my_mana() >= game.portal_cost - game.get_myself().mana_per_turn) and game.can_build_portal_at(loc_to_build_portal)):
                        ...
                        if enemy_elf_to_elf and enemy_elf_to_elf[0].distance(elf) < elf.attack_range + 2*elf.max_speed and enemy_elf_to_elf[0] in attack.values():
                            sortdebug(game,"Go to enemy elf that attack my elf ")
                            mapElves[elf] = enemy_elf_to_elf[0]    
                        else:
                            sortdebug(game,"Go to enemy castle for build a portal 1 ")
                            mapElves[elf] = loc_to_build_portal
                    else:
                        enemy_elf_to_elf = sorted(game.get_enemy_living_elves(),key = lambda e: e.distance(elf))
                        ...
                        if enemy_portal_to_me and not enemy_portal_to_me[0] in my_tornado_turrgets.values():
                            ...
                            if not other_tornado_attack:
                                ...
                                mapElvesPerTurns[elf] = mapElves[elf]
                        if elf not in mapElves.keys():
                            sortdebug(game,"Go to enemy castle-3")
                            mapElves[elf] = game.get_enemy_castle()
                            mapElvesPerTurns[elf] = mapElves[elf]
                    ...
                    if enemy_portal_to_me :
                        ...
                        mapElvesPerTurns[elf] = mapElves[elf]
                    if elf not in mapElves.keys():
                        sortdebug(game,"Go to enemy castle-4")
                        mapElves[elf] = game.get_enemy_castle()
                        if not portal_in_loc:
                            mapElvesPerTurns[elf] = loc_to_build_portal
...
def dangerous_portal(game,elf):
    ...
    if len(game.get_all_my_elves())<len(game.get_all_enemy_elves()):
        return None
    dangerous_portals=[]
    #Enemy portal is dangerous when : It close to much to my castle,It far from the middle,It can destroy my castle by low health lava 
    my_portal_to_enemy_castle=sorted(game.get_my_portals(),key=lambda portal:portal.distance(game.get_enemy_castle()))
    enemy_portal_to_my_castle=sorted(game.get_enemy_portals(),key=lambda portal:portal.distance(game.get_my_castle()))
    for portal in game.get_enemy_portals():
...
def dangerous_elf(game,elf):
    ...
    dangerous_elf=[]
    #Enemy elf is dangerous when: She is closer to my castle than me ,She is  far to much from the middel
    my_elf_to_my_castle=[]
    for elf3 in game.get_my_living_elves():
        if ((elf3 not in mapElvesPerTurns.keys()) or (mapElvesPerTurns[elf3] not in game.get_enemy_living_elves())) and not(dangerous_portal1[elf3]!= None and attack[elf3] == dangerous_portal1[elf3]):
            my_elf_to_my_castle.append(elf3)   
    my_elf_to_my_castle.sort(key=lambda elf1:elf1.distance(game.get_my_castle()))
    enemy_elf_to_my_castle=[]
    for enemy in game.get_enemy_living_elves():
        if (enemy not in mapElvesPerTurns.values() or (elf in mapElvesPerTurns.keys() and mapElvesPerTurns[elf] == enemy)) :
            enemy_elf_to_my_castle.append(enemy)   
    enemy_elf_to_my_castle.sort(key=lambda elf1:elf1.distance(game.get_my_castle()))
    
    if my_elf_to_my_castle and enemy_elf_to_my_castle:
        loc = game.get_my_castle()
        if  my_elf_to_my_castle[0].distance(loc)+4*game.elf_max_speed>enemy_elf_to_my_castle[0].distance(loc):
            my_elf_to_enemy_elf=my_elf_to_my_castle
            my_elf_to_enemy_elf.sort(key=lambda elf1:elf1.distance(enemy_elf_to_my_castle[0]))
            if not( enemy_elf_to_my_castle[0] in enemy_elf_with_a_chaser) and my_elf_to_enemy_elf and my_elf_to_enemy_elf[0].distance(elf)<=elf.max_speed*4 and ((not(my_elf_to_my_castle[0].distance(enemy_elf_to_my_castle[0])<5*elf.max_speed and my_elf_to_my_castle[0].distance(game.get_my_castle())<enemy_elf_to_my_castle[0].distance(game.get_my_castle())))or (spell_casted(game,enemy_elf_to_my_castle[0])=="speed up" or spell_casted(game,enemy_elf_to_my_castle[0])=="both of them" )):
                dangerous_elf.append(enemy_elf_to_my_castle[0])
    for enemy in game.get_enemy_living_elves():
    ...
    for enemy_elf in dangerous_elf:
        if enemy_elf.distance(game.get_enemy_castle())<diagonal_line/4 +game.castle_size and not (my_portal_to_enemy_castle and my_portal_to_enemy_castle[0].distance(game.get_enemy_castle())<enemy_elf.distance(game.get_enemy_castle())):# and not (my_elf_to_my_castle and enemy_elf.distance(game.get_my_castle()) < my_elf_to_my_castle[0].distance(game.get_my_castle())):# and (spell_casted(game,enemy_elf)!="speed up" and  spell_casted(game,enemy_elf)!="both of them" ):
...
def good_locations_for_building_fountain(game):
	#find good places to build fountain - behind of portals
    limit=limits(game)
    locations=[]
    for portal in game.get_my_portals():
        for y in limit:
...
def good_locations_for_portal_for_defend_on_fountain(game):
	#find good places to build portal - in front of fountains
    limit=limits(game)
    locations=[]
...
def if_enemy_destroy_my_fountain(game):
    global if_enemy_destroy_my_mana_fountain
    loc_fountain1=[]
    ...
    if fountain and fountain[0].distance(game.get_my_castle())<fountain[0].distance(game.get_enemy_castle()) and game.get_my_portals():
        if_enemy_destroy_my_mana_fountain=True   
...
def when_I_go_to_volcano(game,elf):
    # return true when elf need to go to volcano
    volcano=my_volcano[elf]
    if volcano==None:
        return False
    if  volcano.damage_by_enemy >volcano.max_health/2 or not game.get_my_portals(): 
        return False
    if len(game.get_my_mana_fountains())> len(game.get_enemy_mana_fountains()) and (elf.distance(volcano)-(volcano.size+elf.attack_range))<diagonal_line/8 <elf.max_speed*10:
        sortdebug( game,"volcano p=2")
        return True
    if game.turn <50 and len(game.get_my_mana_fountains())>= len(game.get_enemy_mana_fountains()):
        sortdebug( game,"volcano p=2")
        return True
    enemy_elf_to_volcano =sorted(game.get_enemy_living_elves(),key=lambda e:e.distance(volcano))
    if (elf.distance(volcano)-(volcano.size+elf.attack_range))<diagonal_line/8 <elf.max_speed*10 and  len(game.get_enemy_living_elves())<len(game.get_my_living_elves()) :
        sortdebug( game,"volcano p=2")
        return True 
        
    PortalToEnemyCastle = sorted(game.get_my_portals(), key = lambda portal: portal.distance(game.get_enemy_castle()))
    EnemyPortalToMyCastle = sorted(game.get_enemy_portals(), key = lambda portal: portal.distance(game.get_my_castle()))
    if PortalToEnemyCastle and EnemyPortalToMyCastle and EnemyPortalToMyCastle[0].distance(game.get_my_castle())>PortalToEnemyCastle[0].distance(game.get_enemy_castle())*2 and (elf.distance(volcano)-(volcano.size+elf.attack_range))<diagonal_line/8 <elf.max_speed*10:
        sortdebug( game,"volcano p=2")
        return True 
        
    if not EnemyPortalToMyCastle and (elf.distance(volcano)-(volcano.size+elf.attack_range))<diagonal_line/6 <elf.max_speed*15:
        sortdebug( game,"volcano p=2")
        return True 
...
def when_I_go_to_alternative_way(game,elf,target):
	#alternative_way is another way to get to destination but this way is far from the enemies
    global alternative_way_for_elf
...
def need_run(game, elf):
	#return object if elf need to escape / return None when elf don't need to escape 
    ...
    if finish_the_game(game) and elf.distance(game.get_enemy_castle()) <= game.castle_size / 2:
        return None
    # stop run if elf with spell and go to enemy fountain
    ...
    if elf == None:
        return None
    # the health of the elf so low better to die
    ...
    dangerous = sorted(danger_ice, key=lambda d: d.distance(elf))
    ...
    if (spell_casted(game, elf) == "invisibility") and not (dangerous and dangerous[0].distance(elf) < dangerous[0].attack_range + dangerous[0].max_speed and elf in Elves_invisibility_turns.keys() and Elves_invisibility_turns[elf] < 2):
        return None
    ...
    counter_enemy = 0
    counter_me = 0
    enemy_elves_near_elf = sorted(game.get_enemy_living_elves(), key=lambda e: e.distance(elf))
    if enemy_elves_near_elf and elf.distance(enemy_elves_near_elf[0]) < elf.attack_range + elf.max_speed:
...
def try_attack(game,elf,for_check):
    ...
    if enemy_attackers_count(game,elf) == 1 and elf not in enemy_ice_turrgets.values():
        ...
        if enemy_elf_to_this_elf and elf.in_attack_range(enemy_elf_to_this_elf[0]) and ((elf.current_health > 3*enemy_elf_to_this_elf[0].current_health) or (not game.get_enemy_portals() and not game.get_enemy_ice_trolls() and elf.current_health>enemy_elf_to_this_elf[0].current_health)):
            ...
            sortdebug(game,"                      *****ATTACK(elf before fountain)*****")
            sortdebug(game," ")
            sortdebug(game,"                      *****Elf Id = "+str(elf.id)+" *****")
            sortdebug(game," ")
    ...
    if optinal_enemy_elves:
        ...
        # if only this elf can attack 
            ...
            return True
    # attack volcano
    if my_volcano[elf]!=None and my_volcano[elf].is_active() and elf.in_attack_range(my_volcano[elf]) and elf in mapElvesPerTurns.keys() and mapElvesPerTurns[elf] == my_volcano[elf] and (spell_casted(game,elf) != "speed up" and spell_casted(game,elf) != "both of them" and  spell_casted(game,elf) != "invisibility"):
        if for_check:
            return my_volcano[elf]
        elf.attack(my_volcano[elf])
...
def sort_enemy_ice(game):
	#return dictionary keys= enemy ice trolls values= ice's destination
...
def sort_my_ice(game):
	#return dictionary keys= my ice trolls values= ice's destination
...
def sort_enemy_tornado(game):
    #return dictionary keys= enemy tornado values= tornado's destination
...
def sort_my_tornado(game):
	#return dictionary keys= my tornado values= tornado's destination
...
def limits(game):
	#return list of locations that in the limits of the map
    limit=[]
...
def distance_point_from_line(location1,location2,point):
	#return the distance from point to the line between location1 to location2
    xloc1=location1.col
    xloc2=location2.col
    yloc1=location1.row
    yloc2=location2.row
...
def attackers_count12(game,enemy):
    ...
    for elf in game.get_my_living_elves():
        if elf.in_attack_range(enemy):
            count +=1
    for ice in game.get_my_ice_trolls(): 
        enemy_ice_near_my_ice = sorted(game.get_enemy_ice_trolls(), key = lambda i: i.distance(ice))
        if ice.distance(enemy)<game.ice_troll_attack_range and ((not enemy_ice_near_my_ice) or ice.distance(enemy)<ice.distance(enemy_ice_near_my_ice[0])):
            count +=1
        elif ice.distance(enemy)<game.ice_troll_attack_range and enemy_ice_near_my_ice and enemy_ice_near_my_ice[0] == enemy:
           count +=1
    return count
...
def enemy_attackers_count(game,my):
    ...
    for elf in game.get_enemy_living_elves():
        if elf.in_attack_range(my):
            count +=1
    for ice in game.get_enemy_ice_trolls(): 
        my_ice_near_enemy_ice = sorted(game.get_my_ice_trolls(), key = lambda i: i.distance(ice))
        if ice.distance(my)<game.ice_troll_attack_range and ((not my_ice_near_enemy_ice) or ice.distance(my)<ice.distance(my_ice_near_enemy_ice[0])):
            count +=1
        elif ice.distance(my)<game.ice_troll_attack_range and my_ice_near_enemy_ice and my_ice_near_enemy_ice[0] == my:
           count +=1
    return count
